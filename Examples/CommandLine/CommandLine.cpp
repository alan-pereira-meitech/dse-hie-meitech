...
namespace CommandLineSample
{
    ...
    private:
        void ParseArguments(array<String^>^ args)
        {
            ...
            {
                _ipAddress = args[0]->Trim();
            }

            if (args->Length > 1)
            {
                int interval;
                if (Int32::TryParse(args[1], interval))
                {
        ...
        {
            Console::WriteLine();
            Console::WriteLine("HBM Automation API DSE Jet command line sample (C++/CLI)");
            Console::WriteLine("Connected to {0} via JetBus", _ipAddress);
            Console::WriteLine();
        ...
        bool TryConnect()
        {
            Console::WriteLine("Connecting to {0} via JetBus...", _ipAddress);
        ...
        BaseWTDevice^ CreateDevice()
        {
            ...
            return gcnew DSEJet(jetConnection, _timerInterval, gcnew ProcessDataReceivedEventHandler(this, &ConsoleRunner::OnProcessDataReceived));
        }
        ...
        void DumpStatus()
        {
            ...
            IProcessData^ data = _device->ProcessData;
        ...
        void OnProcessDataReceived(Object^ sender, ProcessDataReceivedEventArgs^ e)
        {
            ...
            IProcessData^ data = e->ProcessData;
